...
def edit_album(request, album_id):
    ...
    if request.method == HTTP_METHOD_POST:
        ...
        if form.is_valid():
            try:
                ...
                if image_file:
                    delete_album_image(album.image_url)
                    image_url = handle_album_image_upload(image_file)
                    album_data['image_url'] = image_url
                else:
                    album_data['image_url'] = album.image_url  

                album_repo.update_album(album_id, album_data) 
    ...
    return render(request, 'album/edit.html', {'form': form, 'album': album,'today':today, 'artists':artists_json})
'''
@login_required
def edit_album(request, album_id):
    album = album_repo.get_album_id(album_id)
